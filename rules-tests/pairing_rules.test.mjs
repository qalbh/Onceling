...
beforeEach(async () => {
  ...
  await testEnv.withSecurityRulesDisabled(async (context) => {
    ...
    // Same request, carrying P2-25's denormalised sender fields.
    await setDoc(doc(db, "pairingRequests", "req-named"), {
      fromUid: BOB,
      toUid: ALICE,
      status: "pending",
      createdAt: new Date(),
...
describe("pairingRequests — participants only", () => {
  ...
  // P2-25 denormalises the sender's name and avatar onto this document so the
  // recipient can render who is asking without a rule that lets anyone read
  // anyone's profile. That only holds if the document itself stays private:
  // otherwise the denormalisation has moved the leak rather than removed it.
  test("the denormalised sender name is not exposed to anyone else", async () => {
    // Eve cannot read it at all, so fromDisplayName is no more reachable than
    // fromUid was. The two people who can read it already know who sent it.
    await assertFails(getDoc(doc(db(EVE), "pairingRequests", "req-named")));
    // And she cannot reach it by querying for the name either.
    await assertFails(
      getDocs(
        query(
          collection(db(EVE), "pairingRequests"),
          where("fromDisplayName", "==", "Bob"),
        ),
      ),
    );
  });

  test("both participants can read the denormalised name", async () => {
    for (const uid of [ALICE, BOB]) {
      const snap = await assertSucceeds(
        getDoc(doc(db(uid), "pairingRequests", "req-named")),
      );
      assert.equal(snap.data().fromDisplayName, "Bob");
    }
  });
